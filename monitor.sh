...
for ip in "${PING_IPS_ARR[@]}"; do
  ...
  handle_status "ping:ip:$ip" "$status"         "✅ PING OK: \`$ip\`"         "❌ PING DOWN: \`$ip\`"
...
for host in "${PING_HOSTS_ARR[@]}"; do
  ...
  handle_status "ping:host:$host" "$status"         "✅ PING OK: \`$host\`"         "❌ PING DOWN: \`$host\`"
...
for url in "${HTTP_URLS_ARR[@]}"; do
  ...
  status=UP
  if [[ ! "$code" =~ ^2|3 ]] && [[ $fails -ge $RETRIES ]]; then
    status=DOWN
  fi

  handle_status "http:$url" "$status"         "✅ HTTP OK: \`$url\` (code $code)"         "❌ HTTP DOWN: \`$url\` (code $code)"
...
for hp in "${SSL_HOSTS_ARR[@]}"; do
  [[ -z "$hp" ]] && continue
  hp=$(echo "$hp" | xargs); [[ -z "$hp" ]] && continue

  host=${hp%%:*}
  port=${hp##*:}
  [[ "$port" == "$host" ]] && port=443

  not_after=$(echo | timeout "$CHECK_TIMEOUT" openssl s_client -servername "$host" -connect "$host:$port" 2>/dev/null         | openssl x509 -noout -enddate 2>/dev/null         | sed -E 's/^notAfter=//') || not_after=""

  if [[ -z "$not_after" ]]; then
    handle_status "ssl:$host:$port" DOWN           "✅ SSL OK (could read cert): \`$host:$port\`"           "❌ SSL CHECK FAILED: \`$host:$port\` (no cert info)"
    continue
  fi
  ...
  now=$(now_epoch)
  if [[ -z "$exp_epoch" ]]; then
    handle_status "ssl:$host:$port" DOWN           "✅ SSL OK (parsed): \`$host:$port\`"           "❌ SSL PARSE FAILED: \`$host:$port\` ($not_after)"
    continue
  fi
  days_left=$(( (exp_epoch - now) / 86400 ))

  if (( days_left <= SSL_ALERT_DAYS )); then
    handle_status "ssl:$host:$port" DOWN           "✅ SSL RECOVERED: \`$host:$port\` now valid > ${SSL_ALERT_DAYS}d"           "❌ SSL EXPIRING: \`$host:$port\` in ${days_left}d (<= ${SSL_ALERT_DAYS}d)"
  elif (( days_left <= SSL_WARN_DAYS )); then
    handle_status "ssl:$host:$port" DOWN           "✅ SSL RECOVERED: \`$host:$port\` now valid > ${SSL_WARN_DAYS}d"           "⚠️  SSL WARNING: \`$host:$port\` in ${days_left}d (<= ${SSL_WARN_DAYS}d)"
  else
    handle_status "ssl:$host:$port" UP           "✅ SSL OK: \`$host:$port\` (${days_left}d left)"           "❌ SSL UNKNOWN: \`$host:$port\`"
  fi
